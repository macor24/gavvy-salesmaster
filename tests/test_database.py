...
def test_database_import():
    """测试数据库模型导入"""
    try:
        from tianlong_salesmaster.core.storage.database import (
...
def test_repository_import():
    """测试 Repository 导入"""
    try:
        from tianlong_salesmaster.core.storage.pgrepo import (
...
def test_migration_import():
    """测试迁移工具导入"""
    try:
        from tianlong_salesmaster.core.storage.migration import (
...
def test_jsonb_support():
    """测试 JSONB 列支持"""
    try:
        from tianlong_salesmaster.core.storage.database import Customer
...
def test_partition_info():
    """测试分区表信息"""
    try:
        from tianlong_salesmaster.core.storage.database import AuditLog
...
if __name__ == "__main__":
    ...
    if passed == len(tests):
        ...
        print("   3. Run: python -c 'from tianlong_salesmaster.core.storage.database import init_database; init_database()'")
        sys.exit(0)
    else:
