...
from tianlong_salesmaster.team_pkg.llm import (
...
class TestLocalLLM(unittest.TestCase):
    ...
    @patch("tianlong_salesmaster.core.llm_engine.subprocess.run")
    @patch("tianlong_salesmaster.core.llm_engine.os.path.exists", return_value=True)
    @patch("tianlong_salesmaster.core.llm_engine.shutil.which", return_value="/usr/bin/llama-cli")
...
class TestFallbackChain(unittest.TestCase):
    ...
    @patch("tianlong_salesmaster.core.llm_engine.urlopen")
    @patch("tianlong_salesmaster.core.llm_engine.time.sleep", return_value=None)  # 跳过重试等待
    def test_circuit_opens_after_threshold(self, mock_sleep, mock_urlopen):
        """连续失败达到阈值后熔断"""
        from urllib.error import URLError
        mock_urlopen.side_effect = URLError("connection refused")
        from tianlong_salesmaster.core.llm_engine import _CIRCUIT_BREAK_THRESHOLD
        chain = FallbackChain(
...
class TestSalesLLMIntegration(unittest.TestCase):
    ...
    def test_sales_llm_importable(self):
        """SalesLLM 可以从两个路径导入"""
        from tianlong_salesmaster.team_pkg.llm import SalesLLM as ViaLlm
        from tianlong_salesmaster.core.llm_engine import SalesLLM as ViaEngine
