"""gavvy_salesmaster.core.api — [已弃用] gavvy HTTP API 服务

⚠️ 此文件已弃用，请使用 app.py（FastAPI 统一服务）。
    保留以兼容旧版调用，新代码请使用:
    
    from gavvy_salesmaster.core.app import app, start_app
    
    CLI: gavvy-sales-fastapi
...
用法:
    ...
    # Python 直接使用
    from gavvy_salesmaster.core.api import start_sales_api
...
API 端点:
    ...
    # ── gavvy 六维引擎 ──
    POST   /api/analyze              — 分析客户输入（gavvy 六维分析）
...
def start_sales_api(
    ...
    print(f"🚀 gavvy API 服务启动: http://{host}:{port}")
...
def main(argv: Optional[list[str]] = None) -> int:
    """CLI 入口: gavvy-sales-api"""
    import argparse

    parser = argparse.ArgumentParser(
        prog="gavvy-sales-api",
        description="gavvy 销售引擎 — gavvy HTTP API 服务",
    )
