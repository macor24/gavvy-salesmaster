"""gavvy_salesmaster.core.storage.db_sqlite — SQLite 回退数据库层

当 PostgreSQL 不可用时，使用 SQLite 作为开发/测试数据库。
保持与 PostgreSQL 层相同的接口设计。

使用方法:
    from gavvy_salesmaster.core.storage.db_sqlite import init_database, get_db_context
...
def migrate_from_json(json_dir: str) -> Dict[str, int]:
    from .migration import migrate_all_from_json
    return migrate_all_from_json(json_dir, get_db_context)
