@echo off
REM 天龙销售宗师 - PostgreSQL 启动脚本 (Windows)
REM 使用方法: 双击运行此脚本

echo ============================================================
echo 天龙销售宗师 - PostgreSQL 数据库设置
echo ============================================================
echo.
...
REM 检查 tianlong-postgres 容器
docker ps -a --format "{{.Names}}" | findstr /C:"tianlong-postgres" >nul 2>&1
if %errorlevel% equ 0 (
    echo [提示] 容器 tianlong-postgres 已存在
    docker start tianlong-postgres >nul 2>&1
    echo [完成] 容器已启动
) else (
...
REM 测试连接
docker exec tianlong-postgres psql -U salesadmin -d tianlong_sales -c "SELECT 1" >nul 2>&1
if %errorlevel% equ 0 (
...
echo   2. 启动服务: tianlong-sales-fastapi
